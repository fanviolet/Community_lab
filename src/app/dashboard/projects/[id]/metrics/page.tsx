...
import { ArrowLeft, BarChart3, Clock, CheckCircle, AlertTriangle } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-0 bg-white shadow-sm ring-1 ring-black/5">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics.completion_percentage.toFixed(0)}%</div>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="flex items-center justify-between text-sm mb-2">
                ...
                <span className="font-medium">{metrics.total_actual_hours}h</span>
              </div>
              {metrics.total_estimated_hours > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span>Efficiency</span>
                  <span className="font-medium">
                    {((metrics.total_estimated_hours / (metrics.total_actual_hours || 1)) * 100).toFixed(0)}%
                  </span>
                </div>
              )}
            </div>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{metrics.completed_milestones}</span>
              </div>
              {metrics.active_milestones + metrics.completed_milestones > 0 && (
                <div className="flex items-center justify-between text-sm mt-2">
                  <span>Milestone Completion</span>
                  <span className="font-medium">
                    {((metrics.completed_milestones / (metrics.active_milestones + metrics.completed_milestones)) * 100).toFixed(0)}%
                  </span>
                </div>
              )}
            </div>
